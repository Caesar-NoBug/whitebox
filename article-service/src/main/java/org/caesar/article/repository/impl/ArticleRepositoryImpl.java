...
public class ArticleRepositoryImpl extends ServiceImpl<ArticleMapper, ArticlePO>
    ...
    public List<Article> getUpdatedArticle(LocalDateTime afterTime) {
        return baseMapper.getUpdatedArticle(afterTime)
                ...
                .map(articlePO -> {
                    ...
                    article.setViewNum(cacheRepo.getLogLogCount(RedisPrefix.ARTICLE_VIEW_COUNT + article.getId()));
                    article.setFavorNum(cacheRepo.getLongValue(RedisPrefix.ARTICLE_FAVOR_COUNT + article.getId()));
                    article.setLikeNum(cacheRepo.getLongValue(RedisPrefix.ARTICLE_LIKE_COUNT + article.getId()));
                    return article;
                })
    ...
    public boolean updateArticle(long userId, Article article) {
        // 删除文章缓存
        cacheRepo.deleteObject(RedisPrefix.CACHE_ARTICLE + article.getId());
